...
| Phase 8 | 稳定性与部署准备 | Done | 测试样例、日志、错误恢复、Docker、本地部署说明 |
...
- `pnpm infra:up` 已验证核心依赖可一条命令启动。
- 后端迁移已验证到 `alembic head`。
- 后端全量测试覆盖 golden sample、job retry、provider 失败恢复、会议删除文件清理。
- 前端 lint、typecheck、test、build 已验证通过。
